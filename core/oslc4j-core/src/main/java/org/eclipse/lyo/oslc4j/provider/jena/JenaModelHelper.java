...
public final class JenaModelHelper {
      ...
      if (responseInfoAbout != null) {
        final Resource responseInfoResource =
            model.createResource(
                responseInfoAbout, model.createProperty(OslcConstants.TYPE_RESPONSE_INFO));
  ...
  public static Object fromJenaResource(final Resource resource, Class<?> beanClass)
    ...
    // Build reification cache for this resource's model
    final Map<org.apache.jena.graph.Triple, List<Statement>> reificationCache =
      ...
      if (setMethod == null) {
        if (RDF_TYPE_URI.equals(uri)) {
          ...
          if (extendedProperties == null) {
            logger.debug(
                "Set method not found for object type: {}, uri: {}", beanClass.getName(), uri);
          } else {
        ...
        for (RDFNode o : objects) {
          ...
          if (o.isLiteral()) {
            ...
            if (URI.class == setMethodComponentParameterClass) {
              ...
              Optional<Class<?>> optionalResourceClass =
                  ResourcePackages.getClassOf(nestedResource, setMethodComponentParameterClass);
              Class<?> resourceClass =
          ...
          if (parameter != null) {
            ...
            if (multiple) {
              List<Object> values =
                  propertyDefinitionsToArrayValues.computeIfAbsent(uri, k -> new ArrayList<>());
  ...
  private static Resource getResource(Model model, Node node) {
    ...
    if (node.isURI()) {
      ...
      s = model.createResource(new AnonId(node.getBlankNodeLabel()));
    } else {
  ...
  /**
   * Builds a cache of all reified statements in the model upfront.
   * This performs a single graph scan to collect all reifications, avoiding repeated
   * graph traversals during unmarshalling.
   *
  ...
  private static void handleExtendedValue(
    ...
    if (value instanceof UnparseableLiteral unparseable) {
      ...
      // TODO: re-check rdf:XMLLiteral generation. Jena 4 said:
      ...
      final Literal xmlString = model.createLiteral(result.getWriter().toString());
  ...
  private static void handleLocalResource(
    ...
    // Doubles need special handling for infinite values.
      ...
      if (aboutURI != null) {
        ...
        nestedResource =
            model.createResource(aboutURI.toString(), model.createProperty(namespace, name));
      } else {
      ...
      if (resource.getURI() == null
          || (resource.getURI() != null
              && !resource.getURI().equalsIgnoreCase(nestedResource.getURI()))) {
  ...
  private static void addReifiedStatements(
    ...
    if (reifiedJenaResource.listProperties().toList().size() == 4) {
      reifiedJenaResource.removeProperties();
        ...
        try {
          ...
          throw new RuntimeException(
              "The custom namespace provider implementation: "
                  + customNamespaceProvider.getName()
                  + ", must have a public no args construtor",
              e);
        } catch (InstantiationException e) {
          throw new RuntimeException(
              "The custom namespace provider must not be a abstract,"
                  + " nor interface class and must have a public no args constructor",
              e);
